      ...
      <div className="ml-[70px] h-screen flex items-center justify-center bg-gray-100">
        <div className="w-screen h-screen px-6 bg-white rounded-lg shadow-lg">
          <div className="flex justify-center items-center my-8">
            <h1 className="text-red-500 text-2xl font-bold">Inventory Management</h1>
          </div>
          <div className="flex flex-wrap gap-4">
            {menuItems.map((item, index) => (
              <Link
                href={item.href}
                key={item.id}
                className={`border w-[20dvw] rounded-lg shadow-md p-4 flex flex-col items-center cursor-pointer ${activeIndex === index ? 'bg-red-500 text-white' : 'bg-white text-zinc-700 border-red-500'
                  }`}
